...
while True:                                     # ずっと繰り返す。抜けるときは break を使う
    print(f"\n1) 追加  2) 一覧  3) 削除  4) 終了 (現在:{len(tasks)}件)")  # メニューを表示（ \n は改行）：追加機能（メニューに現在の件数を表示）
    choice = input("番号を入力: ")                # キーボードから文字列を受け取る

    if choice == "1":                           # もし「1」が入力されたら（== は「等しい」）
        title = input("タスク名: ")              # 追加したいタスクの名前をもらう
        if title == "":                         # 追加機能（空文字を拒否）：入力の"妥当性チェック"は、値を使う前におこなうのが基本
            print("空のタスクは追加できません。")
        else:                                   # タスク名の値が空でなければ
        ...
        if not tasks:                           # リストが空なら（not は「でない」）    
            ...
            print(f"合計: {len(tasks)}件")       # 追加機能（合計件数を表示）：forの外に設置しないと毎行表示されてしまう。
